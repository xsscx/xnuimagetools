...
###############################################################
#
# build-native.sh — Native arm64 clang build for xnuimagetools
#
...
SRC_DIR="$REPO_ROOT/XNU Image Fuzzer/XNU Image Fuzzer"
...
BINARY="$BUILD_DIR/xnuimagetools"
...
# ── Build ────────────────────────────────────────────────────
do_build() {
  banner "Building xnuimagetools (arm64 Mac Catalyst, ASAN+UBSAN+Coverage)"
...
# ── Run ──────────────────────────────────────────────────────
do_run() {
  banner "Running xnuimagetools under sanitizers with coverage"
  ...
  UBSAN_OPTIONS="print_stacktrace=1:halt_on_error=0" \
    "$BINARY" 2>&1 | tee /tmp/tools-run.log
  ...
  ASAN_OPTIONS="detect_leaks=0:halt_on_error=0" \
  UBSAN_OPTIONS="print_stacktrace=1:halt_on_error=0" \
    "$BINARY" --pipeline "$PIPELINE_DIR" --iterations 2 2>&1 | tee -a /tmp/tools-run.log || true
  ...
  ASAN_HITS=$(grep -c "ERROR: AddressSanitizer" /tmp/tools-run.log 2>/dev/null || true)
  ASAN_HITS="${ASAN_HITS:-0}"
  UBSAN_HITS=$(grep -c "runtime error:" /tmp/tools-run.log 2>/dev/null || true)
